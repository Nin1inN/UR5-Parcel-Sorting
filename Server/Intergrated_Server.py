...
#This function will call findLocation and convertToWorldLocation to get TCP values
#Expected value at end is an array of 6 values. 
#Need to test this function
def calculateTCPValues():
    global stream
...
#                id_matrix[ids[i]] = 1
#        
#        findLocation(id_matrix)
#        
#        for i in range(0, len(id_matrix)):
#            id_matrix[i] = 0
    
    
     #Rest of data will contain TCP values
    jsonResult = {"first":"TCP Values"}
    send(jsonResult, 1)
...
def convertToWorldLocation(x, y):
    ...
    TCPValuesArray[0] = worldPosition[0]
    TCPValuesArray[1] = worldPosition[1]
...
def findLocation(id_matrix):
    global row_position_average 
    global column_position_average
...
#    
    ...
    if(len(row_position_average)  == 20):
       ...
       convertToWorldLocation(row_position_average, column_position_average)
       row_position_average = np.zeros(0)
       column_position_average = np.zeros(0)
    ...
    while(True):
        ...
        if(jsonReceived["first"] == "Client 1"):
            ...
            if(jsonReceived["second"] == "Ready for TCP Values"):
                ...
                #Run this function in a thread, avoid blocking
                t1 = threading.Thread(target = calculateTCPValues )
